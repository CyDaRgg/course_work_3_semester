...
void Ghost::update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, Ghost& i_ghost_0, Pacman& i_pacman)
{
	...
	//========================================================================================
	 // состояние(ключевые моменты для перехода состояний)                                    |
	 //выбрать цель,                                                                          |
	 //коллизия со стеной,                                                                    |
	 //выбор кротчайшего пути при (0,2 режимах),                                              |
	 //выбор рандомного пути при режиме (1),                                                  |
	 //возможность попасть в тунель,                                                          |
	 //взаимодействие с пакманом                                                              |
	 //========================================================================================
		
	bool move = 0;

	//If this is greater than 1, that means that the gohst has reached the intersection.(перекресток)
	...
	{                                                                      // не были напуганы(0) и съел таблетку
		frightened_speed_timer = GHOST_FRIGHTENED_SPEED;

		frightened_mode = 1;
	}
	else if (1 == frightened_mode && 0 == i_pacman.get_energizer_timer())  // были напуганы(1) и не глотал
	{
		frightened_mode = 0;
	}
	...
	if (2 == frightened_mode && 0 == position.x % GHOST_ESCAPE_SPEED && 0 == position.y % GHOST_ESCAPE_SPEED)
		// 
	//if (2 == frightened_mode) // застрянут вдоль или поперек в режиме 2
	{                                                                      // в ужасе(2) и их съел пакман
	...
	walls[0] = map_collision(I_Use_Door::No_, use_door, speed + position.x, position.y, i_map);
	walls[1] = map_collision(I_Use_Door::No_, use_door, position.x, position.y - speed, i_map);
	walls[2] = map_collision(I_Use_Door::No_, use_door, position.x - speed, position.y, i_map);
	walls[3] = map_collision(I_Use_Door::No_, use_door, position.x, speed + position.y, i_map);

	if (1 != frightened_mode)                            // 0 2
	{
		...
		{
			//Gohsts can't turn back! (Unless they really have to) // если справа есть стена, а призрак идет влево, то зачем тратить ресурсы на обработку коллизии
			if (a == (2 + direction) % 4)        //a(w)|    0         1          2            3
			{                                    // --------------------------------------------
				continue;                        //(d)0|    2         2         (2)           2
			}                                    //   1|    3         3          3           (3)
			else if (0 == walls[a])              //   2|   (0)        0          0            0
			{                                    //   3|    1        (1)         1            1
				if (4 == optimal_direction)// пока нету выбора 
				{
					optimal_direction = a;
				}
		...
		else  // такого в данной карте нету, но это на случай, если зашел в тупик
		{
			if (4 == optimal_direction)
			{
				
				direction = (2 + direction) % 4;   // такого в данной карте нету, но это на случай, если зашел в тупик
			}
			else
			{
				direction = optimal_direction;
	...
	else // режим 1(съел таблетку)
	{
		
		unsigned char random_direction = rand() % 4; // 0 1 2 3
		...
		if (0 == frightened_speed_timer)
		{
			...
			else
			{
				...
				direction = (2 + direction) % 4;           // опять же, такого нету в данной игре, так как нету тупиков
			}
		}
		else
		{
...
void Ghost::update_target(unsigned char i_pacman_direction, const Position& i_ghost_0_position, const Position& i_pacman_position)
{
	...
	else
	{
		if (0 == movement_mode) //The scatter mode
		{
			...
			case 0:
			{
				target.x = CELL_SIZE * (MAP_WIDTH - 1); target.y = 0;
			...
			case 2:
			{
				target.x = CELL_SIZE * (MAP_WIDTH - 1); target.y=CELL_SIZE* (MAP_HEIGHT - 1);

				break;
			}
			case 3:
			{
				target.x = 0; target.y= CELL_SIZE* (MAP_HEIGHT - 1);
			}
			}
		}
		else //The chase mode
...
bool Ghost::pacman_collision(const Position& i_pacman_position)
{
	...
	if (pow(position.x - i_pacman_position.x, 2) < pow(CELL_SIZE, 2) && pow(position.y - i_pacman_position.y, 2) < pow(CELL_SIZE, 2))
		{
