...
bool map_collision(bool i_collect_pellets, bool i_use_door, short i_x, short i_y, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map)
{
	...
	for (unsigned char a = 0; a < 4; a++)
	{
		...
		{
			x = static_cast<short>(floor(cell_x)); //The floor() function in C++ returns the largest possible integer value which is less than or equal to the given argument.
		...
		{
			x = static_cast<short>(ceil(cell_x));//The ceil() function in C++ returns the smallest possible integer value which is greater than or equal to the given argument. It is defined in the cmath header file.
			y = static_cast<short>(floor(cell_y));
		...
		if (0 <= x && 0 <= y && MAP_HEIGHT > y && MAP_WIDTH > x)// надо, так как если за границу карты заходить, то выход за массив будет
		{
